...
namespace sc
{
	...
	public sealed class KeyGen
	{
		...
		{
			return DeriveKey(ASCIIEncoder(password), keySize, salt, iterationCount);
		}
		
		public static byte[] DeriveKey(byte[] password, int keySize, byte[] salt, int iterationCount)
		{
			...
			for(int i = 0; i < iterationCount; i++)
			{
				if(keySize > 32)
				{
					data = SHA512.MessageSHA512(data);
				}
				else
				{
