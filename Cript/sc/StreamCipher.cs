...
namespace sc
{
	public interface StreamCipherMonitor
	{
		bool ShouldStop();
		void Progress(long progress, long length);
	}
	...
	public class StreamCipher
	{
		...
		public static StreamCtx MakeStreamCtx(IBlockCipher ibc, byte[] key, byte[] iv, StreamCipher.Mode mode)
		{
			...
			ctx.Ibc = ibc;
			if(!ctx.Ibc.InitCipher(key)) return null;
			ctx.IV = iv;
			ctx.Mode = mode;
			ctx.MakeReadOnly();
			return ctx;
		...
		public static void Encrypt(StreamCtx ctx, Stream instr, Stream outstr, StreamCipherMonitor monitor)
		{
			...
			bool ivInited = false;
			int i = 0;
			if(monitor != null) monitor.Progress(0, instr.Length);
			do
			{
				if(monitor != null)
				{
